...
export class EdituseraddressComponent implements OnInit {
  ...
  getAddress(id) {
    this.loading = true;
    this.userService.getUserAddress(id).subscribe(data => {
      if (data.success === true) {
        if (data.data) {
          this.useraddress = data.data;
          const payload: any = {};
          payload.provincename = '';
          this.getProvince(this.useraddress.country_code, payload, (error, result) => {
          ...
          const payload1: any = {};
          payload1.cityname = '';
          this.getCity(this.useraddress.province_id, payload1, (error, result) => {
          ...
          this.getDistrict(this.useraddress.city_id, payload2, (error, result) => {
          ...
          this.toastr.error('Address id is incorrect in the URL');
        }
      } else {
  ...
  updateUseraddress(formdata: any): void {
    this.loading = true;
    formdata.addressid = this.addressid;
    formdata.addressmail = this.useraddress.address_mail;
    this.userService.updateUserAddress(this.userObj.userid, formdata)
      .subscribe(data => {
        this.loading = false;
  ...
  countryChangeEvent(): void {
    ...
    this.getProvince(res[1], payload, (error, result) => {
      if (error) {
        ...
        this.getCity(this.province[0].province_id, payload, (errorc, resultc: any[]) => {
          if (errorc) {
